...
export async function generateMetadata({ params }: { params: { courseId: string } }) {
  ...
  return {
    title: course?.title ? `${course.title} | LMS Legends` : "Course | LMS Legends",
  };
}
...
export default async function CourseIdPage({
  ...
  // TODO: Check if user has purchased the course (Phase 5)
    ...
    <div className="flex-1">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row gap-8 items-start">
           ...
           <div className="w-full md:w-80 lg:w-96 rounded-2xl bg-white dark:bg-zinc-950 p-6 border border-zinc-200 dark:border-zinc-800 shadow-xl lg:-mb-32 z-10 sticky top-24">
              <div className="relative aspect-video rounded-xl overflow-hidden bg-zinc-100 dark:bg-zinc-900 mb-6">
                 {course.image_url ? (
                    <Image fill className="object-cover" alt={course.title} src={course.image_url} />
                 ) : (
              ...
              <div className="space-y-4">
                 ...
                 <p className="text-xs text-zinc-500 text-center">30-day money-back guarantee</p>
              </div>
           </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-8 py-16">
          <div className="max-w-3xl">
             <h2 className="text-2xl font-bold mb-8 tracking-tight">Course Curriculum</h2>
             
             <div className="space-y-4">
                {course.modules?.map((module: any, index: number) => (
                    <div key={module.id} className="border border-zinc-200 dark:border-zinc-800 rounded-xl overflow-hidden">
                        <div className="bg-zinc-50 dark:bg-zinc-900/50 p-4 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between font-semibold text-lg">
                           ...
                           <span className="text-sm font-normal text-zinc-500">{module.lessons?.length || 0} lessons</span>
                        </div>
                        <div className="divide-y divide-zinc-200 dark:divide-zinc-800">
                           {module.lessons?.map((lesson: any) => (
                               <div key={lesson.id} className="p-4 flex items-center justify-between hover:bg-zinc-50 dark:hover:bg-zinc-900/30 transition">
                           ...
                           {(!module.lessons || module.lessons.length === 0) && (
                               <div className="p-4 text-zinc-500 text-sm italic">No lessons in this module.</div>
                           )}
                        </div>
                    </div>
                ))}
                {(!course.modules || course.modules.length === 0) && (
                    <div className="text-zinc-500 italic">No modules available yet.</div>
                )}
             </div>
          </div>
      </div>
